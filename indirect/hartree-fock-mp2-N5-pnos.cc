...
std::tuple<int, int, mcc::Tensor4<double>, Matrix> mp2_proc(std::tuple<int, int, 
                                                          Matrix, Matrix,
                                                          mcc::Tensor4<double>> hf_vals) {
  std::tuple<int, int, mcc::Tensor4<double>, Matrix> mp2_vals;
  ...
  mp2_vals = std::tie(nao, nocc, mo_mo_mo_mo, eps);
...
// Form element (a,b) of the T^ij matrix
double comp_t_ab(int i, int j, int a, int b, mcc::Tensor4<double> mo_mo_mo_mo, Matrix eps) {

  double k = mo_mo_mo_mo(a,i,b,j);
  double elem = -k/(eps(a,a) + eps(b,b) - eps(i,i) - eps(j,j));
...
Matrix form_T_ij(int i, int j, int nao, int nocc,
                  mcc::Tensor4<double> mo_mo_mo_mo, Matrix eps) {
  ...
  for (int a=nocc; a<nao; ++a) {
    for (int b=nocc; b<nao; ++b) {
      T_ij(a-nocc,b-nocc) = comp_t_ab(i, j, a, b, mo_mo_mo_mo, eps);
    }
  }
...
Matrix form_D_ij(int i, int j, int nao, int nocc,
  ...
  int delta = kron_delta(i,j);
  double coeff = 1.0/(1+delta);
  Matrix D_ij = coeff*(T_ij_tilde.transpose()*T_ij + T_ij_tilde*T_ij.transpose());

  return D_ij;
}

// For given i,j pair, diagonalize the D^ij matrix
tuple<Matrix, Matrix> diagonalize_D(Matrix D_ij) {

  tuple<Matrix, Matrix> pno_vals;
  Eigen::SelfAdjointEigenSolver<Matrix> es;
  es.compute(D_ij);
  Matrix pnos = es.eigenvectors();
  Matrix occ = es.eigenvalues().asDiagonal();

  pno_vals = std::tie(pnos, occ);

  return pno_vals;

}

// Loop through i,j pairs to calculate D^i,j matrices
int make_D(int nao, int nocc, mcc::Tensor4<double> mo_mo_mo_mo, Matrix eps) {

  for (int i=0; i<nocc; ++i) {
    for (int j=0; j<nocc; ++j) {
      Matrix D_ij = form_D_ij(i, j, nao, nocc, mo_mo_mo_mo, eps);
      cout << "\nFor i = " << i << " and j = " << j << " the D^ij matrix is\n"
      << D_ij << endl;
    }
  }
  return 0;
...
// Call PNO process
int pno_proc(std::tuple<int, int, mcc::Tensor4<double>, Matrix> mp2_vals) {

  int nao = std::get<0>(mp2_vals);
  int nocc = std::get<1>(mp2_vals);
  mcc::Tensor4<double> mo_mo_mo_mo = std::get<2>(mp2_vals);
  Matrix eps = std::get<3>(mp2_vals);

  int D = make_D(nao, nocc, mo_mo_mo_mo, eps);
...
int main() {
  ...
  std::tuple<int, int, mcc::Tensor4<double>, Matrix> mp2_vals = mp2_proc(hf_vals);
  ...
  // Call the PNO process
  int PNOs = pno_proc(mp2_vals);

  //double elem = comp_t_ab(0, 0, 5, 5, mo_mo_mo_mo, eps);
  //cout << elem << endl;

  //Matrix T_ij = form_T_ij(0, 0, nao, nocc, mo_mo_mo_mo, eps);
  //cout << T_ij << endl;
